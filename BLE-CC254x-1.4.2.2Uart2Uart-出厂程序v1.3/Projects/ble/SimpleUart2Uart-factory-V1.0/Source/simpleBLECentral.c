...
// Scan duration in ms
#define DEFAULT_SCAN_DURATION                4000  //默认扫描时间 ms
...
// Default GAP bonding I/O capabilities//是否具有显示和输入密码能力宏定义
...
static uint8 simpleBLEScanIdx;  //扫描索引
...
void SimpleBLECentral_Init( uint8 task_id )
{
  ...
  // Setup GAP
  GAP_SetParamValue( TGAP_GEN_DISC_SCAN, DEFAULT_SCAN_DURATION );//设置普通发现模式下的扫描时间-4000ms
  GAP_SetParamValue( TGAP_LIM_DISC_SCAN, DEFAULT_SCAN_DURATION );//设置限制发现模式下的扫描时间-4000ms 
  //GGS_SetParameter( GGS_DEVICE_NAME_ATT, GAP_DEVICE_NAME_LEN, (uint8 *) simpleBLEDeviceName );
  GGS_SetParameter( GGS_DEVICE_NAME_ATT, GAP_DEVICE_NAME_LEN, simpleBle_GetAttDeviceName() );//设置蓝牙名称


  // Setup the GAP Bond Manager
  
  {
    uint32 passkey = simpleBle_GetPassword();//读取密码
    uint8 pairMode;// = DEFAULT_PAIRING_MODE;
    uint8 mitm = DEFAULT_MITM_MODE;//FALSE 不需要密码
    uint8 ioCap = DEFAULT_IO_CAPABILITIES;//只有显示能力，所以不能手动输入密码，由蓝牙自行生成密码去匹配
    ...
    ---------------amomcu.com-------------------------    
    */
    uint8 bonding = FALSE;//不绑定

//#if defined( BLE_BOND_PAIR )
    if(simpleBle_GetIfNeedPassword())//是否需要密码配对
    {
        pairMode = GAPBOND_PAIRING_MODE_INITIATE;   //Don't wait, initiate a pairing request or slave security request
    }
    else
    {
        pairMode = GAPBOND_PAIRING_MODE_WAIT_FOR_REQ;//Wait for a pairing request or slave security request
    }    
//#endif   
    //设置上面信息
 ...
 // HalLedSet( (HAL_LED_1 | HAL_LED_2), HAL_LED_MODE_OFF );
   ...
   {
      AH_NPI_WriteTransport("Central_Init_OK\r\n");
   }
...
//     LCD_WRITE_STRING( "--simpleBLEStartScan", HAL_LCD_LINE_2 );
    ...
    if ( !simpleBLEScanning/* & simpleBLEScanRes == 0 */)
    {
      ...
      AH_NPI_WriteTransport("Start Scanning...\r\n");
    }
    else
    {
      AH_NPI_WriteTransport("In Scanning...\r\n");
    }
}
...
uint16 SimpleBLECentral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    // Start the Device
    VOID GAPCentralRole_StartDevice( (gapCentralRoleCB_t *) &simpleBLERoleCB );//注册2个回调函数。  1.RSSI callback，2.Event callback

    // Register with bond manager after starting device
    GAPBondMgr_Register( (gapBondCBs_t *) &simpleBLEBondCB );//注册2个回调函数。1.Passcode callback,  2.Pairing state callback.
...
//        LCD_WRITE_STRING( "Enable Notice\n", HAL_LCD_LINE_1);           
        ...
        AH_NPI_WriteTransport("Connected\r\n");
    }
    else
    {
  ...
  {
#if 1   
    uint16 totalbytes = qq_total();//查询缓存区字节数
    uint8 numBytes;
...
//         osal_set_event( simpleBLETaskId, SBP_DATA_EVT );
  ...
  if ( events & SBP_PERIODIC_EVT )//自循环事件，定时处理
  {
    // Restart timer
    ...
    simpleBLE_performPeriodicTask();//LED显示
    ...
    if ( simpleBLEState == BLE_STATE_CONNECTED )
    {
      ...
      {
        simpleBLERssi = TRUE;
        GAPCentralRole_StartRssi( simpleBLEConnHandle, DEFAULT_RSSI_PERIOD );//开始定期获取Rssi
    ...
    else
    {
        if(simpleBLERssi)
        {
            GAPCentralRole_CancelRssi( simpleBLEConnHandle );   //取消定期获取Rssi     
...
//      LCD_WRITE_STRING_VALUE( "Write Error", status, 10, HAL_LCD_LINE_1 );
  ...
  else if ( ( pMsg->method == ATT_HANDLE_VALUE_NOTI ) )   //通知
  {
    if( pMsg->msg.handleValueNoti.handle == simpleBLECharHd6/*0x0035*/)   //CHAR6的通知  串口打印
    {
        ...
        {        
            AH_NPI_WriteTransport(bdAddr2Str(pMsg->msg.handleValueNoti.pValue)); 
...
void simpleBLECentraDisplaNextPeriAddr(bool nextFalg)
{
   ...
   if ( simpleBLEScanRes > 0 )
   {
        ...
        //AH_NPI_WriteTransport(bdAddr2Str(simpleBLEScanIdx));
     
        AH_NPI_WriteTransport("[");
        AH_NPI_WriteTransport(AH_bdAddr2Str(&simpleBLEScanIdx));
        AH_NPI_WriteTransport("]");
        //NPI_WriteTransport((uint8*)str, osal_strlen(str)); 
        AH_NPI_WriteTransport(":");
        AH_NPI_WriteTransport(bdAddr2Str( simpleBLEDevList[simpleBLEScanIdx].addr));
        AH_NPI_WriteTransport("\r\n");
...
static uint8 simpleBLECentralEventCB( gapCentralRoleEvent_t *pEvent )
{
  ...
  {
    case GAP_DEVICE_INIT_DONE_EVENT:  // 初始化完成标志事件
      {
        AH_NPI_WriteTransport(bdAddr2Str(pEvent->initDone.devAddr ));//串口输出本机地址
        AH_NPI_WriteTransport("\r\n");
      }
      break;

    case GAP_DEVICE_INFO_EVENT://记录从机广播事件
      {
        // if filtering device discovery results based on service UUID
        if ( DEFAULT_DEV_DISC_BY_SVC_UUID == TRUE )
        {
          if ( simpleBLEFindSvcUuid( SIMPLEPROFILE_SERV_UUID,//查找想要的UUID
                                     pEvent->deviceInfo.pEvtData,
                                     pEvent->deviceInfo.dataLen ) )
          {
            simpleBLEAddDeviceInfo( pEvent->deviceInfo.addr, pEvent->deviceInfo.addrType );//添加 未 添加过的设备地址
          }
        }
      }
      break;
      
    case GAP_DEVICE_DISCOVERY_EVENT://扫描完成，显示结果
      {
        ...
        AH_NPI_WriteTransport("Devices Found:");
        AH_NPI_WriteTransport(AH_bdAddr2Str(&simpleBLEScanRes));
        AH_NPI_WriteTransport("\r\n");
...
//            LCD_WRITE_STRING_VALUE( "g_Central_connect_cmd = ", g_Central_connect_cmd, 10, HAL_LCD_LINE_2 );
            ...
            if(g_Central_connect_cmd  == BLE_CENTRAL_CONNECT_CMD_DISC)
            {
                ...
                for(i = 0; i < simpleBLEScanRes; i++)
                {
                   ...
                   AH_NPI_WriteTransport(bdAddr2Str( simpleBLEDevList[i].addr ));//串口显示扫描地址
                   AH_NPI_WriteTransport("\r\n");
                }
                
                // 搜索完成后返回 OK+DISCE。
                AH_NPI_WriteTransport("END_Scanning\r\n");             
           
            }
            else if(g_Central_connect_cmd  == BLE_CENTRAL_CONNECT_CMD_CONNL|| 
                     g_Central_connect_cmd  == BLE_CENTRAL_CONNECT_CMD_CONN )//连接指定最后成功的从机 \ 连接新从机
            {
...
//                LCD_WRITE_STRING( "Continue scanning", HAL_LCD_LINE_1 );
    ...
    case GAP_LINK_ESTABLISHED_EVENT://建立连接事件
      {
    ...
    case GAP_LINK_TERMINATED_EVENT://连接超时事件
      {
        simpleBLEState = BLE_STATE_IDLE;
        simpleBLEConnHandle = GAP_CONNHANDLE_INIT;
        //simpleBLERssi = FALSE;
  ...
  while ( pData < pEnd )
  {
    ...
    if ( adLen > 0 )
    {
      ...
      if ( adType == GAP_ADTYPE_16BIT_MORE || adType == GAP_ADTYPE_16BIT_COMPLETE )
      {
        ...
        {
          // Check for match
          if ( pData[0] == LO_UINT16(uuid) && pData[1] == HI_UINT16(uuid) )//当前位置的【0】和【1】
          {
            // Match found
            return TRUE;
          }
